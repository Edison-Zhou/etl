package cn.whaley.datawarehouse.market.dim

import java.text.SimpleDateFormat
import java.util.{Calendar, Date}

import cn.whaley.datawarehouse.BaseClass
import cn.whaley.datawarehouse.market.dim.constants.Constants._
...
/**
  ...
  * @date 2019/9/5 15:41
  */
abstract class DmDimensionBase extends BaseClass {
  ...
  def readSource(readSourceType: Value): DataFrame = {
    ...
    else if (readSourceType == hive) {
      DataExtractUtils.readFromDimension(sqlContext,sourceTable)
    }
    else {
      null
    }
  ...
  def readSourceIncr(readSourceType: Value): DataFrame = {
    ...
    if (readSourceType == null || readSourceType == jdbc) {
      ...
      sqlContext.table(sourceTable).where(s"$sourceTimeCol >= '$dateStr'")
    }
    else null
  }
  ...
  private def checkPrimaryKeys(sourceDf: DataFrame, primaryKeys: List[String]) = {
  ...
  override def transform(params: Params, filteredSourceDf: DataFrame): DataFrame = {
    ...
    val onlineDimensionDir = DM_DIMENSION_HDFS_BASE_PATH + File.separator + dimensionName

    if(!HdfsUtil.IsDirExist(onlineDimensionDir)){
      HdfsUtil.createDir(onlineDimensionDir)
    }
    //首次创建维度
    if (!HdfsUtil.fileIsExist(onlineDimensionDir, "_SUCCESS")) { //判断维度表路径下有没有success文件
      ...
      val result = DataFrameUtil.dfZipWithIndex(
        DataFrameUtil.addDimTime(
          ...
          DmDimensionBase.defaultValidTime,
          null),
        columns.skName
      )
      return result
    ...
    if (!fullUpdate && newColumns.nonEmpty) throw new RuntimeException("有新增列，必须用全量更新模式运行")
    ...
    val changedTrackingColumnDf =
      filteredSourceDf.as("b").join(
        ...
        if (columns.trackingColumns == null || !columns.trackingColumns.exists(!newColumns.contains(_))) "1!=1" else {
    ...
    val extendDf =
      if (columns.trackingColumns == null || columns.trackingColumns.isEmpty) DataFrameUtil.addDimTime(addDf, DmDimensionBase.defaultValidTime, null) else DataFrameUtil.addDimTime(addDf, DmDimensionBase.defaultValidTime, null).unionAll(
        //因为追踪列变化而新增的行
        DataFrameUtil.addDimTime(
    ...
    //合并上述形成最终结果
    val offset =
      if (df.count() > 0) df.agg(max(columns.skName)).first().getLong(0) else {
        println("WARN! 原维度表为空")
        0
      }
    ...
    if (columns.addColumns != null) columns.addColumns.foreach(column =>
      unionAllResult = unionAllResult.withColumn(column.name, column.udf(column.inputColumns.map(col): _*))
    )
    //关联其他维度，支持雪花模型
    val linkDimension = parseDimension(unionAllResult, columns.linkDimensionColumns, columns.skName, columns.invalidTimeKey,DM_DIMENSION_HDFS_BASE_PATH)

    var result =
      if (linkDimension == null) unionAllResult else {
  ...
  private def backup(p: Params, df: DataFrame, dimensionType: String): Unit = {
    ...
    val onLineDimensionDir = DM_DIMENSION_HDFS_BASE_PATH + File.separator + dimensionType
    val onLineDimensionBackupDir = DM_DIMENSION_HDFS_BASE_PATH_BACKUP + File.separator + date + File.separator + dimensionType
    val onLineDimensionDirTmp = DM_DIMENSION_HDFS_BASE_PATH_TMP + File.separator + dimensionType
    val onLineDimensionDirDelete = DM_DIMENSION_HDFS_BASE_PATH_DELETE + File.separator + dimensionType
    ...
    if (isOnlineFileExist) {
      ...
      if (isBackupExist) println("数据已经备份,跳过备份过程")
      else {
    ...
    val partition = Math.max(1, (total_count / DM_THRESHOLD_VALUE).intValue())
