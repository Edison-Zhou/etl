...
object LayoutPage extends DimensionBase {
  ...
  columns.trackingColumns = List("page_name","area_name","sub_area_name")
  ...
  sourceDbFromDetailLayoutPosition = MysqlDB.medusaCms("mtv_layoutPosition", "id", 1, 3000, 20)
  sourceDbFromDetailLayoutPositionItem = MysqlDB.medusaCms("mtv_layoutPositionItem", "id", 1, 3000, 20)

  dimensionName = "dim_medusa_layout_page"
  ...
  override def readSource(SourceType: Value): Dataset[Row] = {
    ...
    /**
      * 取出所需列生成新的df
      * union了两部分的数据
      * 1）针对大首页的"我的"和"分类"区域精确到location_code
    ...
    sqlContext.sql(
      """
        ...
        |and (b.module_code = 'editor_my' or b.module_code = 'editor_classification')
        |union
        |select distinct b.page_code,a.title as page_name,b.module_code as area_code,d.title as area_name
        ...
        |and (b.module_code != 'editor_my' and b.module_code != 'editor_classification')
