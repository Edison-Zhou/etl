...
import cn.whaley.datawarehouse.util.UdfUtils
...
/**
  * Created by zhu.bingxin on 2018/09/10.
  * 电视猫4X的路径解析
  */
object Path4XParserUtils {
  ...
  def getLayoutPageInfo(path: Seq[Row]) = {
    ...
    if (HOME_PAGE_LIST.contains(page_type) && access_area != null && access_area != "") {
      ...
      homePageCode = page_type match {
        case LAUNCHERACTIVITY => "newHomePage4"
        case _ => channelType.concat("HomePage4")
      }
    ...
    if (homePageCode == "memberHomePage4") homePageCode = "vipHomePage4" //针对会员首页pageCode做特殊处理
    if (!StringUtils.isEmpty(accessArea) && accessArea.split("_")(0) == "ysm") homePageCode = "ysmHomePage1" //针对优视猫首页pageCode做特殊处理
  ...
  def getChannelPlayEntrance(path: Seq[Row]): String = {
    ...
    var channelName: String = channelType match {
      case "movie" => "4.0电影分类入口"
      case "tv" => "4.0电视剧分类入口"
      case "zongyi" => "4.0综艺分类入口"
      case "kids" => "4.0少儿分类入口"
      case "comic" => "4.0动漫分类入口"
      case "game" => "4.0游戏电竞分类入口"
      case "hot" => "4.0资讯分类入口"
      case "interest" => "4.0奇趣分类入口"
      case "sports" => "4.0体育分类入口"
      case "member" => "4.0会员分类入口"
      case "jilu" => "4.0纪实分类入口"
      case "mv" => "4.0音乐分类入口"
      case "cantonese" => "4.0粤语分类入口"
      case "xiqu" => "4.0戏曲分类入口"
      case "sohuyueting" => "4.0悦厅专区分类入口"
      case _ => null
    }
    ...
    //少儿的各专区页、少儿大首页、游戏体育大首页）
      ...
      result = "4.0“我的”会员频道入口"
    }
      // add by wangning 190716
      //4.0首页搜索入口
    else if(entrancePage == SEARCHACTIVITY && entrance.schema.exists(_.name == "access_area") && entrance.getAs[String]("access_area") == "keyWord"){
      result = entrancePage+"keyWord"
    }
    else if(entrancePage == VODACTIVITY && entrance.schema.exists(_.name == "access_area") && entrance.getAs[String]("access_area") == "m_Livehome4x" && channelType=="webcast"){
      result = "m_Livehome4xwebcast"
    }
    else {
      result = "4.0其他"
    }
