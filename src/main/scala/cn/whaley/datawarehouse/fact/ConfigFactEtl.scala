...
import cn.whaley.datawarehouse.fact.moretv.util.{Play3xCombineUtils, Play4xCombineUtils}
import cn.whaley.datawarehouse.global.Globals._
import cn.whaley.datawarehouse.global.LogConfig
import cn.whaley.datawarehouse.util.{DateFormatUtils, HdfsUtil, Params, UdfUtils}
...
/**
  * Created by zhu.bingxin on 2018/8/29.
  */
object ConfigFactEtl extends FactEtlBase with LogConfig {

  private val interfaceUrl = "http://bigdata-app.whaley.cn/matrix/api/remote/fact/get?id="
  ...
  def initParam(params: Params): Unit = {
    ...
    //    factTime = if (tableData.getString("factTime").length > 0)
    //      tableData.getString("factTime") else "concat(dim_date, '', dim_time)"

    factTime = tableData.getString("factTime")
  ...
  //保留哪些列，以及别名声明
  partition = 0
  interfaceData.getJSONArray("factColumns") //获取factColumns配置值
    .toArray.foreach(data => {
    val columnName = JSON.parseObject(data.toString).getString("columnName")
  ...
  //解析事实字段中使用到的维度表
  val factExpressions = interfaceData.getJSONArray("factColumns").map(data => JSON.parseObject(data.toString).getString("expression"))
...
/**
 ...
 def readSource(sourceDate: String, sourceHour: String,params: Params): DataFrame = {
  ...
  private def readSourceFromParquet(params: Params): DataFrame = {
    ...
    val completePath = getPath(params,FACT_HDFS_BASE_PATH_COMPLETE)
  ...
  private def readSourceFromOds(sourceDate: String, sourceHour: String):DataFrame={
    ...
    if (extractSql.contains ("log_medusa_main3x_play") )
      mergerDf = Play3xCombineUtils.get3xCombineDataFrame (mergerDf, sqlContext)
    else if (extractSql.contains ("log_medusa_main4x_play") )
      mergerDf = Play4xCombineUtils.get4xCombineDataFrame (mergerDf, sqlContext)
    //当读入的数据源为空时，退出应用程序
    if (mergerDf.count () == 0) {
  ...
  private def updateFactColumns(params: Params, sourceDf: DataFrame): DataFrame = {
    ...
    //读取事实表
    val factTablePath = getPath(params, FACT_HDFS_BASE_PATH)
    ...
    if (factCol.exists(_.endsWith("_sk")) && dimensionColumns.nonEmpty) {
      ...
      dimensionJoinDf = parseDimension(sourceFilterDf, dimensionColumnList, uniqueLogIdColumn, factTime,DIMENSION_HDFS_BASE_PATH)
  ...
  private def updateOrAddColumns(params: Params, sourceDf: DataFrame): DataFrame = {
    ...
    val completePath = getPath(params, FACT_HDFS_BASE_PATH_COMPLETE_TMP)
    val completePathExist = HdfsUtil.IsDirExist(completePath)
    if (completePathExist) {
  ...
  /**
    * 修复sql，所有的表的参数进行替换，针对4X播放表需要额外读第二天的2小时的数据
    *
    * @param sql
    * @param date
    * @return
